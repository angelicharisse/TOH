...
public class Level_01_Scene extends Scene{
	...
	public Level_01_Scene()
	{
		...
		levelBgTextureRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(levelBgAtlas,activity.getBaseContext(),"ocean.png",0,0);
	...
	public void loadResources()
	{
		...
		// 2 - Add the towers
			...
			// 3 - Create the rings
			Ring ring1 = new Ring(1, 139, 229, this.mRing1, activity.getVertexBufferObjectManager()) {
			    @Override
			...
			Ring ring2 = new Ring(2, 118, 268, this.mRing2, activity.getVertexBufferObjectManager()) {
			    @Override
			    public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
			        if (((Ring) this.getmStack().peek()).getmWeight() != this.getmWeight())
			            return false;
			...
			Ring ring3 = new Ring(3, 97, 313, this.mRing3, activity.getVertexBufferObjectManager()) {
			    @Override
			    public boolean onAreaTouched(TouchEvent pSceneTouchEvent, float pTouchAreaLocalX, float pTouchAreaLocalY) {
			        if (((Ring) this.getmStack().peek()).getmWeight() != this.getmWeight())
			            return false;
        ...
        if(mStack3.size() == 3){
                activity.runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            mydb.addScore(1, 3, moves, formattedTimeString);
                            mydb.close();
